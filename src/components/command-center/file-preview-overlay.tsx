...
const PreviewButton = Button as any;
const PreviewCard = Card as any;
const PreviewCardContent = CardContent as any;
const PreviewScrollArea = ScrollArea as any;
...
type MarkdownAnnotationLeaveAction = "" | "cancel-edit" | "close-preview" | "close-preview-confirm" | "discard-markdown-annotations" | "discard-markdown-annotations-and-close";
...
function renderShortcutTooltip(label: string, shortcut: string, hint: (shortcut: string) => string) {
  return (
    <div className="space-y-0.5">
      <div>{label}</div>
      <div className="text-[11px] text-muted-foreground">{hint(shortcut)}</div>
    </div>
  );
}
...
function SpreadsheetPreview({ preview, resolvedTheme = "light" }: { preview: PreviewLike; resolvedTheme?: string }) {
  ...
  return (
    <div className={cn("overflow-hidden rounded-xl border", isDark ? "border-border/70 bg-background/80" : "border-slate-200 bg-white")}>
      ...
      <PreviewScrollArea className="max-h-[70vh]">
...
export function ImagePreviewOverlay({ image, onClose }: { image: PreviewLike; onClose?: () => void }) {
  ...
  const clampScale = useCallback((value) => Math.min(4, Math.max(0.5, value)), []);
  const clampOffset = useCallback((candidate, nextScale = scale, nextRotation = rotation) => {
    if (nextScale <= 1) {
      return { x: 0, y: 0 };
    }
  ...
  const updateScale = useCallback((updater) => {
    setScale((current) => {
  ...
  }, [clampOffset, clampScale, rotation]);
  const handleZoomIn = useCallback(() => updateScale((current) => current + 0.25), [updateScale]);
  const handleZoomOut = useCallback(() => updateScale((current) => current - 0.25), [updateScale]);
  ...
  }, [image.path, messages.inspector.previewErrors.revealInFileManagerFailed, openingInFileManager]);
      ...
      <div className="absolute inset-x-0 bottom-0 flex justify-center px-6 pb-6 pt-6" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-center gap-1 rounded-full border border-white/10 bg-black/60 px-2 py-1.5 shadow-2xl backdrop-blur">
          <PreviewTooltip>
            ...
            <PreviewTooltipContent>{renderImageShortcutTooltip(messages.inspector.previewActions.imageRotateLeft, "Q")}</PreviewTooltipContent>
          </PreviewTooltip>
          <PreviewTooltip>
            <PreviewTooltipTrigger asChild>
              <button
            ...
            <PreviewTooltipContent>{renderImageShortcutTooltip(messages.inspector.previewActions.imageRotateRight, "W")}</PreviewTooltipContent>
          </PreviewTooltip>
          <PreviewTooltip>
            <PreviewTooltipTrigger asChild>
              <button
            ...
            <PreviewTooltipContent>{renderImageShortcutTooltip(messages.inspector.previewActions.imageZoomOut, "-")}</PreviewTooltipContent>
          </PreviewTooltip>
          ...
          <PreviewTooltip>
            ...
            <PreviewTooltipContent>{renderImageShortcutTooltip(messages.inspector.previewActions.imageResetZoom, "0")}</PreviewTooltipContent>
          </PreviewTooltip>
          <PreviewTooltip>
            <PreviewTooltipTrigger asChild>
              <button
            ...
            <PreviewTooltipContent>{renderImageShortcutTooltip(messages.inspector.previewActions.imageZoomIn, "=/+")}</PreviewTooltipContent>
          </PreviewTooltip>
          <PreviewTooltip>
            <PreviewTooltipTrigger asChild>
              <button
            ...
            <PreviewTooltipContent>{renderImageShortcutTooltip(messages.inspector.previewActions.revealInFileManager(fileManagerLabel), "O")}</PreviewTooltipContent>
          </PreviewTooltip>
        </div>
      </div>
    </div>
...
export function FilePreviewOverlay({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event) => {
      ...
      if (isMarkdownAnnotationMode || isEditing) {
        setPendingLeaveAction(editSessionDirty ? "close-preview" : "close-preview-confirm");
        return;
      }
      if (editSessionDirty) {
  ...
  const markdownAnnotationToggleLabel = isMarkdownAnnotationMode
    ...
    : messages.inspector.previewActions.annotationUpdate;
  const markdownAnnotationToggleTooltip = isMarkdownAnnotationMode
    ? messages.inspector.previewActions.annotationUpdateActiveTooltip
    : messages.inspector.previewActions.annotationUpdateTooltip;
  ...
  const handleMarkdownAnnotationSubmit = useCallback(async ({ prompt }: { prompt: string }) => {
    ...
    try {
      ...
      setMarkdownAnnotationSubmitError("");
      const preparedSend = onSendPreparedPrompt(prompt, { shouldAppendPromptHistory: true });
      setIsMarkdownAnnotationMode(false);
      setMarkdownAnnotationCount(0);
      setMarkdownAnnotationSubmitError("");
      setPendingLeaveAction("");
      onClose?.();
      void Promise.resolve(preparedSend).catch((error) => {
        console.error("Failed to dispatch markdown annotation prompt", error);
      });
    } catch (error) {
  ...
  const handleRequestCancelEditing = useCallback(() => {
    ...
    handleCancelEditing();
  }, [editSessionDirty, handleCancelEditing]);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event) => {
      ...
      const isAnnotationShortcut =
        ...
        && !event.repeat
        && !event.isComposing
        && (event.code === "KeyA" || normalizedKey === "a");
      ...
      if (isAnnotationShortcut && showMarkdownAnnotationToggle) {
        if (isEditableElement(activeElement) || isEditableElement(eventTarget)) {
          return;
        }
        event.preventDefault();
        event.stopPropagation();
        handleStartMarkdownAnnotationMode();
        return;
      }
  ...
  }, [applePlatform, canEditPreview, handleSaveEditing, handleStartEditing, handleStartMarkdownAnnotationMode, isEditing, preview, showMarkdownAnnotationToggle]);
  ...
  const leaveDialogTitle = pendingLeaveAction === "cancel-edit"
    ...
    : pendingLeaveAction === "close-preview-confirm"
      ? messages.inspector.previewActions.closePreviewConfirm.title
  ...
  const leaveDialogDescription = pendingLeaveAction === "discard-markdown-annotations" || pendingLeaveAction === "discard-markdown-annotations-and-close"
    ...
    : pendingLeaveAction === "close-preview-confirm"
      ? messages.inspector.previewActions.closePreviewConfirm.description
  ...
  const leaveDialogConfirmLabel = pendingLeaveAction === "cancel-edit"
    ...
    : pendingLeaveAction === "close-preview-confirm"
      ? messages.inspector.previewActions.closePreviewConfirm.confirm
    : pendingLeaveAction === "discard-markdown-annotations"
          ...
          <div className={cn("flex items-start justify-between gap-4 border-b px-6 py-4", isDark ? "border-white/10" : "border-slate-200")}>
            <div className="flex min-w-0 items-start gap-3">
              <PreviewTooltip>
                ...
                <PreviewTooltipContent>{isFullscreen ? messages.inspector.previewActions.restore : messages.inspector.previewActions.maximize}</PreviewTooltipContent>
              </PreviewTooltip>
            ...
            <div className="flex items-center gap-2">
                ...
                <>
                  ...
                  >
                    {filePreviewFontSizeOptions.map((option) => {
                      ...
                      return (
                        <PreviewTooltip key={option.value}>
                          <PreviewTooltipTrigger asChild>
                            <button
                              type="button"
                              className={cn(
                          ...
                          <PreviewTooltipContent>{messages.inspector.previewActions.previewFontSizeOptionTooltip(label)}</PreviewTooltipContent>
                        </PreviewTooltip>
                      );
                    })}
                  </div>
              ...
              {canEditPreview ? (
                isEditing ? (
                  ...
                  <PreviewTooltip>
                    ...
                    <PreviewTooltipContent>{renderShortcutTooltip(messages.inspector.previewActions.editFile, editShortcutLabel, messages.theme.shortcutHint)}</PreviewTooltipContent>
                  </PreviewTooltip>
                )
              ) : null}
                  ...
                  <PreviewTooltipTrigger asChild>
                    <PreviewButton
                      ...
                      className={cn(
                        ...
                        previewToolbarSurfaceClassName,
                        isMarkdownAnnotationMode
                          ? "border-primary/70 bg-primary text-primary-foreground shadow-[0_10px_24px_rgba(150,91,34,0.18)] hover:border-primary/80 hover:bg-primary/92 hover:text-primary-foreground focus-visible:ring-primary/45"
                          : previewToolbarInteractiveClassName,
                      )}
                    ...
                    >
                      {isMarkdownAnnotationMode ? <X className="h-3.5 w-3.5" /> : <Pencil className="h-3.5 w-3.5" />}
              ...
              >
                <PreviewTooltip>
                  ...
                  <PreviewTooltipContent>{messages.inspector.previewActions.revealInFileManager(fileManagerLabel)}</PreviewTooltipContent>
                </PreviewTooltip>
                {showVsCodeButton ? (
                  <>
                    ...
                    <PreviewTooltip>
                      ...
                      <PreviewTooltipContent>{messages.inspector.previewActions.openInCodeEditor}</PreviewTooltipContent>
                    </PreviewTooltip>
                  </>
                ) : null}
              </div>
      ...
      {pendingLeaveAction ? (
        <>
          ...
          <div className="fixed inset-0 z-[141] flex items-center justify-center px-4">
            ...
            >
              <PreviewCardContent className="px-5 py-4">
                <div className="space-y-2">
                ...
                <div className="mt-4 flex justify-end gap-2">
                  <PreviewButton type="button" variant="outline" size="sm" onClick={() => setPendingLeaveAction("")}>
                    {messages.inspector.previewActions.unsavedChanges.keepEditing}
                  </PreviewButton>
                  <PreviewButton type="button" size="sm" onClick={handleConfirmLeave}>
