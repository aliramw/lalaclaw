...
const markdownShellClassName =
  "text-[13px] leading-6 [&_a]:text-primary [&_a]:no-underline hover:[&_a]:underline " +
  ...
  "[&_p]:mb-2 [&_ul]:my-2 [&_ul]:list-disc [&_table]:my-2 [&_table]:w-full [&_table]:border-collapse " +
  "[&_table]:overflow-hidden [&_table]:rounded-md [&_thead]:bg-muted/40 [&_th]:border [&_th]:border-border " +
...
export function MarkdownContent({ content, className }) {
  ...
  return (
    ...
    >
      <LazyMarkdownRenderer content={text} className={className} shellClassName={markdownShellClassName} />
    </Suspense>
  );
}
