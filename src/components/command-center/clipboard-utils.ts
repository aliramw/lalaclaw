type ClipboardUploadEntry = { kind: "upload"; file: File };
type ClipboardSourcePathEntry = { kind: "sourcePath"; sourcePath: string };
type ClipboardEntry = ClipboardUploadEntry | ClipboardSourcePathEntry;
...
type ClipboardLikeApi = {
  writeText?: (text: string) => Promise<void>;
  read?: () => Promise<ClipboardLikeItem[]>;
};

export async function copyTextToClipboard(content: unknown) {
  const clipboard =
...
function getClipboardApi(): ClipboardLikeApi | null {
  return (
    (typeof window !== "undefined" && window.navigator?.clipboard)
    || (typeof navigator !== "undefined" && navigator.clipboard)
    || null
...
export function createClipboardUploadEntriesFromFiles(files: File[] = []): ClipboardUploadEntry[] {
...
export async function buildClipboardPasteRequestEntries(entries: ClipboardEntry[] = []) {
