...
function hasLocalLivePendingAssistant(localMessages = [], pendingEntry = null, busy = false) {
...
function findPendingAssistantMessage(messages = [], pendingEntry = null) {
...
function stabilizeHydratedConversation(hydratedConversation = [], localMessages = []) {
...
export function useRuntimeSnapshot({
  ...
  const recoveredPendingProgressRef = useRef({});
  ...
  const handleWsMessage = useCallback((payload) => {
    ...
    if (payload.type === 'session.sync' && payload.session) {
      ...
      const localMessages = messagesRef.current || [];
      const conversationKey = createConversationKey(currentSession.sessionUser, currentSession.agentId);
      const localPendingEntry = pendingChatTurnsRef.current[conversationKey] || derivePendingEntryFromLocalMessages(localMessages) || null;
  ...
  }, [applyIncrementalConversation, applySnapshot, clearRecoveredPendingSettleTimeout, i18n.common.idle, i18n.sessionOverview.fastMode.on, recoveringPendingReply, scheduleRecoveredPendingSettle, setAvailableAgents, setAvailableModels, setBusy, setFastMode, setFilesFromSnapshot, setModel, setSession]);
