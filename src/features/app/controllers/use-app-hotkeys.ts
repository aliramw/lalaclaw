...
type UseAppHotkeysInput = {
  handleActivateAdjacentChatTab?: (direction: -1 | 1) => void;
  handleActivateChatTabByIndex?: (index: number) => void;
  handleReset: () => Promise<unknown>;
  promptRef: RefObject<HTMLTextAreaElement | null>;
  setPromptVisible: (value: string) => void;
...
export function useAppHotkeys({
  ...
  const onResetHotkey = useEffectEvent((event: KeyboardEvent) => {
  ...
  const onThemeHotkey = useEffectEvent((event: KeyboardEvent) => {
  ...
  const onChatTabHotkey = useEffectEvent((event: KeyboardEvent) => {
  ...
  const onAdjacentChatTabHotkey = useEffectEvent((event: KeyboardEvent) => {
    const normalizedKey = String(event.key || "").trim();
  ...
  const onPromptCharacterHotkey = useEffectEvent((event: KeyboardEvent) => {
