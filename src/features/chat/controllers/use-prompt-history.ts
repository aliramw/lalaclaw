...
type PromptTextareaElement = HTMLTextAreaElement & {
  selectionEnd: number;
  selectionStart: number;
  value: string;
};

type PromptKeyboardEventLike = {
  altKey?: boolean;
  ctrlKey?: boolean;
  currentTarget: PromptTextareaElement;
  isComposing?: boolean;
  key?: string;
  keyCode?: number;
  metaKey?: boolean;
...
function isImeComposingEvent(event: PromptKeyboardEventLike | null | undefined) {
  return Boolean(
    event?.isComposing
  ...
  const handlePromptKeyDown = (event: PromptKeyboardEventLike) => {
    ...
    if (canBrowseUp || canBrowseDown) {
      ...
      if (event.key === "ArrowUp") {
        ...
        setPromptHistoryNavigation((current) => (current ? { ...current, index: nextIndex } : current));
        setPrompt(history[nextIndex] || "");
        window.requestAnimationFrame(() => moveCaretToEnd(promptRef.current));
        return;
      }
