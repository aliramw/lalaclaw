...
- 在 `Workspace Files` 中，右鍵資料夾即可把剪貼簿中的上傳檔案或已複製的本機檔案直接貼到該資料夾
- 目錄貼上成功後，樹狀清單會重新整理對應資料夾，新寫入的檔案也會回流到目前工作階段的檔案列表
- 檢查器中的資料夾路徑仍會直接打開 Finder / Explorer / 系統檔案管理器，而不會嘗試做內嵌預覽
