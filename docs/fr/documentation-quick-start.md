...
Si vous disposez d'une machine distante que OpenClaw peut piloter et à laquelle vous pouvez aussi vous connecter en SSH, vous pouvez laisser OpenClaw installer et démarrer LalaClaw à distance, puis y accéder localement avec un transfert de port SSH.
...
1. OpenClaw clone le dépôt sur la machine distante
2. OpenClaw installe les dépendances et démarre l'application
3. LalaClaw écoute sur `127.0.0.1:3000` sur la machine distante
4. Vous transférez ce port distant vers votre machine locale avec SSH
5. Vous ouvrez l'adresse locale transférée dans votre navigateur
...
- Dans ce mode, votre `127.0.0.1:3000` local pointe en pratique vers le `127.0.0.1:3000` de la machine distante
...
- Si le port local `3000` est déjà utilisé, vous pouvez choisir un autre port local comme `3300:127.0.0.1:3000`, puis ouvrir `http://127.0.0.1:3300`
...
Si vous préférez une version publiée précise, comme `2026.3.17-7` :
