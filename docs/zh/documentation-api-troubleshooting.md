...
- 在 `Environment` 里，绝对文件路径会打开预览，但目录路径会刻意跳过预览，直接打开系统文件管理器
- 如果你期待日志目录或工作区根目录以内联方式预览，这其实是当前的预期行为
...
### 为什么在 Environment 里能看到目标，但某些操作仍然是禁用的

常见原因：

- 当前应用可以读取远端 OpenClaw 目标，但该操作被故意限制在本机安全链路内

建议检查：

- `OPENCLAW_BASE_URL` 是否指向非 loopback 主机
- `Environment` 面板里是否已经把当前目标标成远端
- 被禁用的是否属于本机专属写操作，例如 install、update、config apply 或 gateway 管理

预期行为：

- 只读诊断可以同时对本机和远端目标开放
- 风险更高的写操作会保持禁用，直到专门的远端运维流程落地
