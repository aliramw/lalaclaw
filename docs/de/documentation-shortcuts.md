[English](../en/documentation-shortcuts.md) | [中文](../zh/documentation-shortcuts.md) | [繁體中文（香港）](../zh-hk/documentation-shortcuts.md) | [日本語](../ja/documentation-shortcuts.md) | [한국어](../ko/documentation-shortcuts.md) | [Français](../fr/documentation-shortcuts.md) | [Español](../es/documentation-shortcuts.md) | [Português](../pt/documentation-shortcuts.md) | [Deutsch](../de/documentation-shortcuts.md) | [Bahasa Melayu](../ms/documentation-shortcuts.md) | [தமிழ்](../ta/documentation-shortcuts.md)

[Zur Startseite](./documentation.md) | [Oberflächenüberblick](./documentation-interface.md) | [Chat, Anhänge und Befehle](./documentation-chat.md) | [Lokale Persistenz und Wiederherstellung](./documentation-persistence.md)

# Tastenkürzel

- Cmd/Ctrl + /: Hilfe zu Tastenkürzeln öffnen
- Cmd/Ctrl + N: neue Sitzung
- Cmd/Ctrl + 1-9: Sitzung per Nummer wechseln
- Cmd/Ctrl + Left: vorheriger Tab
- Cmd/Ctrl + Right: nächster Tab
- Shift + Cmd/Ctrl + F: Systemtheme
- Shift + Cmd/Ctrl + L: helles Theme
- Shift + Cmd/Ctrl + D: dunkles Theme

Modus Enter zum Senden:

- Enter: senden
- Shift + Enter: Zeilenumbruch

Modus Doppeltes Enter zum Senden:

- Doppelt Enter: senden
- Shift + Enter: senden
- Enter: Zeilenumbruch