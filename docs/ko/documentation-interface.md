...
- 개발 모드에서는 오른쪽 아래에 현재 브랜치, worktree, 포트, 경로를 보여주는 떠 있는 배지가 나타납니다
- 배지는 접거나 펼칠 수 있고, 브라우저를 벗어나지 않고도 대상 worktree와 대상 브랜치를 선택할 수 있습니다
- 이 배지에서 개발 서비스를 즉시 재시작할 수 있으며, 다른 브랜치나 worktree를 선택하면 먼저 전환한 뒤 미리보기 복구를 기다립니다
