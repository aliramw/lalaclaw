...
- `Workspace Files` 에서 디렉터리를 우클릭하면 클립보드 업로드나 복사한 로컬 파일을 그 폴더에 바로 붙여넣을 수 있습니다
- 디렉터리 붙여넣기가 성공하면 해당 폴더가 새로고침되고, 새로 저장된 파일도 현재 세션 파일 목록에 반영됩니다
- inspector 안의 폴더 경로는 계속해서 인라인 미리보기 대신 Finder / Explorer / 시스템 파일 관리자를 직접 엽니다
