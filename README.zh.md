...
如果你已经有一台由 OpenClaw 管理的远端机器，并且你也能通过 SSH 登录这台机器，那么你可以直接让 OpenClaw 从 GitHub 安装这个项目、在远端启动它，然后通过 SSH 端口转发在本地访问这个 dashboard。
...
2. OpenClaw 在远端安装依赖并启动 LalaClaw。
3. 应用在远端机器的 `127.0.0.1:3000` 上监听。
...
- 这种用法下，你本地的 `127.0.0.1:3000` 实际上转发到远端机器的 `127.0.0.1:3000`
- 应用进程、OpenClaw 配置、transcript、日志和工作区都在远端机器上，不在你的本地电脑上
- 这种方式比直接把 dashboard 暴露在公网更安全，因为否则任何知道这个地址的人，都可以在没有密码的情况下直接使用这个控制台
- 如果你本地的 `3000` 端口已被占用，可以改用 `3300:127.0.0.1:3000`，然后打开 `http://127.0.0.1:3300`
