export const THINK_MODES = ['off', 'minimal', 'low', 'medium', 'high', 'xhigh', 'adaptive'];

type SessionStoreOptions = {
  getDefaultAgentId: () => string;
  getDefaultModelForAgent: (agentId: string) => string;
  resolveCanonicalModelId: (value: string, localConfig?: Record<string, unknown> | null) => string;
};
...
export function extractAgentIdFromNativeSessionKey(sessionUser = '') {
  const match = String(sessionUser || '').trim().match(/^agent:([^:]+):/);
  return match?.[1] ? String(match[1]).trim() : '';
}

export function createSessionStore({ getDefaultAgentId, getDefaultModelForAgent, resolveCanonicalModelId }: SessionStoreOptions) {
  const sessionPreferences = new Map<string, SessionPreferences>();
  const localSessionConversation = new Map<string, Array<Record<string, unknown>>>();
  const localSessionFileEntries = new Map<string, Array<Record<string, unknown>>>();
  ...
  function setSessionPreferences(sessionUser = 'command-center', next: SessionPreferences = {}) {
    const key = normalizeSessionUser(sessionUser);
  ...
  function appendLocalSessionConversation(sessionUser = 'command-center', entries: SessionConversationEntry[] = []) {
    const key = normalizeSessionUser(sessionUser);
  ...
  function appendLocalSessionFileEntries(sessionUser = 'command-center', entries: SessionFileEntry[] = []) {
    const key = normalizeSessionUser(sessionUser);
