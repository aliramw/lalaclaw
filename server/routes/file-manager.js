...
function getClipboardFileExtension(mimeType = '') {
  return clipboardMimeExtensionMap[String(mimeType || '').trim().toLowerCase()] || '';
}
...
function createFileManagerHandlers({
  execFileAsync,
  parseRequestBody,
  sendJson,
  ...
  async function handleFileManagerRename(req, res) {
    try {
      ...
      sendJson(res, error?.isClientError ? 400 : 500, {
        ok: false,
  ...
  async function handleFileManagerPaste(req, res) {
    try {
      ...
      sendJson(res, error?.isClientError ? 400 : 500, {
        ok: false,
        error: error.message || 'Paste into directory failed',
      });
    }
  }
