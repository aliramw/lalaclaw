...
type ValidatedAbsolutePath = { path: string } | { error: string };
type ValidatedTargetPath = { path: string; stat: Stats } | { error: string };
type LooseRecord = Record<string, any>;
type ClipboardMimeType = keyof typeof clipboardMimeExtensionMap;
type ClipboardEntry = {
  dataUrl?: string;
  kind?: string;
  mimeType?: string;
...
type ParseRequestBody = (req: IncomingMessage) => Promise<FileManagerRequestBody | LooseRecord | null | undefined>;
...
function validateAbsolutePath(candidate: unknown, invalidMessage: string): ValidatedAbsolutePath {
...
function getClipboardFileExtension(mimeType = '') {
  ...
  if (normalizedMimeType in clipboardMimeExtensionMap) {
    return clipboardMimeExtensionMap[normalizedMimeType as ClipboardMimeType];
  }
  return '';
}
...
function resolveUniqueDestinationPath(directoryPath: string, fileName: string) {
...
function buildSavedFilePayload(filePath: string) {
...
function hasValidationError(value: ValidatedAbsolutePath | ValidatedTargetPath): value is { error: string } {
  return typeof (value as { error?: string }).error === 'string';
}

export function createFileManagerHandlers({
  execFileAsync,
  parseRequestBody,
  sendJson,
  platform = process.platform,
}: FileManagerHandlerOptions) {
  function validateTargetPath(candidate: unknown): ValidatedTargetPath {
  ...
  function validateTargetDirectory(candidate: unknown): ValidatedTargetPath {
  ...
  function saveClipboardUploadEntry(directoryPath: string, entry: ClipboardEntry, index: number) {
    const { buffer, mimeType } = parseClipboardDataUrl(entry?.dataUrl);
    const requestedName = sanitizeClipboardFileName(entry?.name);
    const destinationPath = resolveUniqueDestinationPath(
      directoryPath,
      requestedName || buildGeneratedClipboardFileName(entry?.mimeType || mimeType, index),
    );
    fs.writeFileSync(destinationPath, buffer);
    return buildSavedFilePayload(destinationPath);
  }

  function copyClipboardSourceEntry(directoryPath: string, entry: ClipboardEntry, index: number) {
    ...
    const destinationPath = resolveUniqueDestinationPath(
      directoryPath,
      requestedName || path.basename(validatedSource.path) || buildGeneratedClipboardFileName('', index),
    );
    fs.copyFileSync(validatedSource.path, destinationPath);
    return buildSavedFilePayload(destinationPath);
  }

  async function handleFileManagerRename(req: IncomingMessage, res: ServerResponse) {
    try {
  ...
  async function handleFileManagerReveal(req: IncomingMessage, res: ServerResponse) {
    try {
      const body = await parseRequestBody(req);
      const validated = validateTargetPath(body?.path);
      if (hasValidationError(validated)) {
  ...
  async function handleFileManagerPaste(req: IncomingMessage, res: ServerResponse) {
    try {
      const body = await parseRequestBody(req);
      ...
      const items = entries.map((entry: ClipboardEntry, index: number) => {
...
export function createFileManagerHandler(options: FileManagerHandlerOptions) {
