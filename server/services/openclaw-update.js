...
function buildStateFromStatus(status = {}, preview = null) {
  return {
    ...
    targetVersion: preview?.targetVersion || status?.availability?.latestVersion || null,
  };
}
...
function createOpenClawUpdateService({
  config,
  execFileAsync,
  ...
  async function loadNextStateAfterCommand() {
    try {
      ...
      return {
        state: null,
        stateError: {
          errorCode: error?.errorCode || 'update_status_failed',
          message: String(error?.message || 'Failed to inspect OpenClaw update state'),
        },
      };
    }
  }
