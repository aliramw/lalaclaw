...
function buildGuidance(actionKey, {
  commandResult,
  healthCheck,
} = {}) {
  const guidance = [];
...
function createOpenClawManagementService({
  config,
  execFileAsync,
  fetchImpl = global.fetch,
  now = () => Date.now(),
  ...
  async function runOpenClawAction(actionKey = '') {
    ...
    const definition = openClawActionDefinitions[normalizedAction];
    if (!definition) {
...
module.exports = {
  createOpenClawManagementService,
