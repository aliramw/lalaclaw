...
function createUnsupportedAuthChoiceError() {
  const error = new Error('Unsupported OpenClaw onboarding auth choice');
  error.statusCode = 400;
  error.errorCode = 'unsupported_auth_choice';
  return error;
}

function createMissingValueError(fieldName = '') {
  const error = new Error(`Missing required onboarding value: ${fieldName}`);
  error.statusCode = 400;
  error.errorCode = 'missing_required_value';
  error.field = fieldName;
  return error;
}

function createUnsupportedSecretInputModeError() {
  const error = new Error('Unsupported OpenClaw onboarding secret input mode');
  error.statusCode = 400;
  error.errorCode = 'unsupported_secret_input_mode';
  return error;
}

function createUnsupportedGatewayAuthModeError() {
  const error = new Error('Unsupported OpenClaw onboarding gateway auth mode');
  error.statusCode = 400;
  error.errorCode = 'unsupported_gateway_auth_mode';
  return error;
}

function createUnsupportedGatewayTokenInputModeError() {
  const error = new Error('Unsupported OpenClaw onboarding gateway token input mode');
  error.statusCode = 400;
  error.errorCode = 'unsupported_gateway_token_input_mode';
  return error;
}

function createUnsupportedDaemonRuntimeError() {
  const error = new Error('Unsupported OpenClaw onboarding daemon runtime');
  error.statusCode = 400;
  error.errorCode = 'unsupported_daemon_runtime';
  return error;
}

function createUnsupportedFlowError() {
  const error = new Error('Unsupported OpenClaw onboarding flow');
  error.statusCode = 400;
  error.errorCode = 'unsupported_onboarding_flow';
  return error;
}
...
function createOpenClawOnboardingService({
  config,
  execFileAsync,
  ...
  async function getOpenClawOnboardingState(options = {}) {
    ...
    if (!statusCommandResult.ok || !statusPayload) {
      const error = new Error('Failed to inspect OpenClaw onboarding state');
      error.statusCode = 500;
      error.errorCode = 'onboarding_state_failed';
      throw error;
    }
