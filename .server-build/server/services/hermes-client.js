...
const FALLBACK_HERMES_CONTEXT_WINDOWS = {
    'gpt-5.4': 1050000,
...
function resolveHermesBin(explicitBin = '') {
    ...
    return [
        ...
        homeDir ? node_path_1.default.join(homeDir, '.local', 'bin', 'hermes') : '',
...
function buildHermesExecEnv(hermesBin, baseEnv = process.env) {
    const values = [
        node_path_1.default.dirname(process.execPath),
        node_path_1.default.isAbsolute(hermesBin) ? node_path_1.default.dirname(hermesBin) : '',
        ...String(baseEnv?.PATH || '').split(node_path_1.default.delimiter),
    ]
        .map((value) => String(value || '').trim())
        .filter(Boolean);
    return {
        ...baseEnv,
        PATH: values.filter((value, index) => values.indexOf(value) === index).join(node_path_1.default.delimiter),
    };
}
...
function parseHermesStatus(stdout = '') {
...
function parseHermesSessionStats(stdout = '') {
    const line = String(stdout || '').trim();
    if (!line) {
        return null;
    }
    const [sessionId = '', model = '', inputTokens = '0', outputTokens = '0', cacheReadTokens = '0', cacheWriteTokens = '0', reasoningTokens = '0', messageCount = '0', startedAt = '', endedAt = '',] = line.split('\t');
...
function createHermesClient({ execFileAsync, HERMES_BIN, PROJECT_ROOT, } = {}) {
    ...
    async function getHermesSessionStats(sessionId = '') {
    ...
    async function getHermesModelContextWindow(model = '') {
