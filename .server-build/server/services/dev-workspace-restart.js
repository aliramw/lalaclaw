...
exports.STATUS_FILENAME = void 0;
exports.createDevWorkspaceRestartService = createDevWorkspaceRestartService;
const node_crypto_1 = __importDefault(require("node:crypto"));
const node_fs_1 = __importDefault(require("node:fs"));
const node_path_1 = __importDefault(require("node:path"));
const node_child_process_1 = require("node:child_process");
exports.STATUS_FILENAME = 'dev-workspace-restart.json';
...
function createDevWorkspaceRestartService({ backendHost, backendPort, fileExists, processEnv = process.env, processExecPath = process.execPath, processPid = process.pid, projectRoot, readJsonIfExists, spawnImpl = node_child_process_1.spawn, spawnSyncImpl = node_child_process_1.spawnSync, stateDir, writeFileSyncImpl = node_fs_1.default.writeFileSync, }) {
    const helperScriptPath = node_path_1.default.join(projectRoot, 'scripts', 'dev-workspace-restart.cjs');
    const statusFilePath = node_path_1.default.join(stateDir, exports.STATUS_FILENAME);
    function isAvailable() {
        return Boolean(fileExists(node_path_1.default.join(projectRoot, 'package.json'))
            && fileExists(node_path_1.default.join(projectRoot, 'vite.config.mjs'))
            && fileExists(node_path_1.default.join(projectRoot, 'server.js'))
    ...
    function listWorktrees() {
        ...
        for (const rawLine of lines) {
            ...
            if (line.startsWith('worktree ')) {
                ...
                current = {
                    path: worktreePath,
                    name: node_path_1.default.basename(worktreePath),
                    branch: '',
                    detached: false,
                };
                continue;
    ...
    function scheduleRestart({ frontendHost, frontendPort, targetBranch, targetWorktreePath, }) {
        if (!isAvailable()) {
        ...
        const restartId = typeof node_crypto_1.default.randomUUID === 'function'
            ? node_crypto_1.default.randomUUID()
            : `${Date.now()}-${node_crypto_1.default.randomBytes(8).toString('hex')}`;
        ...
        const child = spawnImpl(processExecPath, [
            helperScriptPath,
