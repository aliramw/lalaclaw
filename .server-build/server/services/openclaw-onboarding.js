...
exports.parseOnboardingHelpCapabilities = parseOnboardingHelpCapabilities;
...
const lalaclaw_service_status_1 = require("./lalaclaw-service-status");
...
function buildStaticOnboardingCapabilities() {
    return {
...
function buildCapabilityDetection({ source = 'static-fallback', reason = '', commandResult = null, detectedAt = '', signature = '', } = {}) {
    return {
...
function intersectPreferredCapabilities(preferredValues = [], detectedValues = []) {
...
function parseOnboardingHelpCapabilities(helpText = '') {
    ...
    return {
        ...
        supportedGatewayAuthModes: intersectPreferredCapabilities(staticCapabilities.supportedGatewayAuthModes, ['off', ...parsedGatewayAuthModes]),
...
function resolveCapabilityValues(capabilities = null) {
    const staticCapabilities = buildStaticOnboardingCapabilities();
...
function resolveSupportedDefaultValue(preferredValue = '', supportedValues = [], fallbackValue = '') {
...
function createApiKeyAuthChoice(flagName) {
    return {
        key: 'apiKey',
        buildArgs: ({ apiKey = '', secretInputMode = DEFAULT_SECRET_INPUT_MODE } = {}) => (secretInputMode === 'plaintext' && apiKey
            ? [flagName, apiKey]
            : []),
    };
}
function createPassthroughAuthChoice() {
    return {
        buildArgs: () => [],
    };
}
const SUPPORTED_AUTH_CHOICES = {
    token: {
        requires: ['tokenProvider', 'token'],
        buildArgs: ({ token = '', tokenExpiresIn = '', tokenProfileId = '', tokenProvider = '', } = {}) => {
            const args = [
                '--token-provider',
                tokenProvider,
                '--token',
    ...
    'custom-api-key': {
        ...
        buildArgs: ({ apiKey = '', customBaseUrl = '', customCompatibility = 'openai', customModelId = '', customProviderId = '', secretInputMode = DEFAULT_SECRET_INPUT_MODE, } = {}) => {
            const args = [
                '--custom-base-url',
                customBaseUrl,
                '--custom-model-id',
    ...
    ollama: {
        buildArgs: ({ customBaseUrl = '', customModelId = '', } = {}) => {
            const args = [];
            if (customBaseUrl) {
...
function normalizeValidationSummary(commandResult, payload) {
...
function resolveSuggestedWorkspace() {
    ...
    return homeDirectory ? node_path_1.default.join(homeDirectory, '.openclaw', 'workspace') : '';
}
...
function normalizeConfigPath(commandResult = {}) {
...
function buildOnboardingState({ capabilities = null, configPath = '', installed = false, validation = null, service = null, } = {}) {
    ...
    return {
        ...
        service: service || (0, lalaclaw_service_status_1.getLalaClawServiceStatus)(),
    };
}
...
function buildOnboardingArgs(options = {}, capabilities = null) {
    const resolvedCapabilities = resolveCapabilityValues(capabilities);
...
function createOpenClawOnboardingService({ config, execFileAsync, fetchImpl = global.fetch, }) {
    ...
    let cachedCapabilitySnapshot = null;
    ...
    async function getSupportedOnboardingCapabilities(signature = '', options = {}) {
    ...
    async function getOpenClawOnboardingState(options = {}) {
