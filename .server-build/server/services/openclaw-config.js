...
exports.openClawConfigFieldDefinitions = exports.CHAT_COMPLETIONS_ENABLED_PATH = exports.GATEWAY_BIND_PATH = exports.AGENT_MODEL_KEY = exports.MODEL_PRIMARY_PATH = void 0;
exports.createOpenClawConfigService = createOpenClawConfigService;
...
exports.MODEL_PRIMARY_PATH = 'agents.defaults.model.primary';
exports.AGENT_MODEL_KEY = 'agentModel';
exports.GATEWAY_BIND_PATH = 'gateway.bind';
exports.CHAT_COMPLETIONS_ENABLED_PATH = 'gateway.http.endpoints.chatCompletions.enabled';
exports.openClawConfigFieldDefinitions = [
    {
        key: 'modelPrimary',
        path: exports.MODEL_PRIMARY_PATH,
        type: 'string',
        allowUnset: true,
    ...
    {
        key: 'gatewayBind',
        path: exports.GATEWAY_BIND_PATH,
        type: 'enum',
    ...
    },
    {
        ...
        path: exports.CHAT_COMPLETIONS_ENABLED_PATH,
        type: 'boolean',
        restartRequired: true,
    },
];
...
function hashContent(value = '') {
    return node_crypto_1.default.createHash('sha256').update(String(value || '')).digest('hex');
}
...
function parseValidationPayload(text = '') {
...
function findAgentModelFieldDefinition(configJson = {}, agentId = '') {
    const normalizedAgentId = String(agentId || '').trim();
    if (!normalizedAgentId) {
        return null;
    }
    const agents = Array.isArray(configJson?.agents?.list) ? configJson.agents.list : [];
    const index = agents.findIndex((agent) => String(agent?.id || '').trim() === normalizedAgentId);
    if (index < 0) {
        return null;
    }
    return {
        key: exports.AGENT_MODEL_KEY,
...
function buildFieldDefinitions(configJson = {}, options = {}) {
    const definitions = [...exports.openClawConfigFieldDefinitions];
...
function buildFieldStateForConfig(configJson = {}, options = {}) {
...
function collectConfiguredModelOptions(configJson = {}) {
    const seen = new Set();
    const ordered = [];
    ...
    (Array.isArray(configJson?.agents?.list) ? configJson.agents.list : []).forEach((agent) => {
...
function resolveImChannelDefaultAgentId(configJson = {}, channel = '') {
    const normalizedChannel = String(channel || '').trim();
    if (!normalizedChannel) {
        return '';
    }
    const bindings = Array.isArray(configJson?.bindings) ? configJson.bindings : [];
    const defaultBinding = bindings.find((binding) => {
    ...
    const fallbackBinding = bindings.find((binding) => String(binding?.match?.channel || '').trim() === normalizedChannel);
    return String(fallbackBinding?.agentId || '').trim();
}
function buildImChannelSummary(configJson = {}) {
    return Object.fromEntries(IM_CHANNEL_DEFINITIONS.map(({ channel, pluginId, channelPath, allowPluginOnly = false }) => {
...
async function ensureFileReadable(filePath = '') {
    ...
    try {
        return await promises_1.default.readFile(normalized, 'utf8');
    }
    catch (error) {
...
function buildValidationSummary(commandResult, payload) {
...
function buildGuidance({ changedFields = [], healthCheck = null, noChanges = false, restartRequested = false, rolledBack = false, validation = null, } = {}) {
    const guidance = [];
    if (rolledBack) {
...
function buildRemoteCommandSummary(method = '', result = null) {
    return {
        ok: true,
        timedOut: false,
        exitCode: 0,
...
function createOpenClawConfigService({ backupStore = null, callOpenClawGateway = null, config, execFileAsync, fetchImpl = global.fetch, now = () => Date.now(), waitImpl = wait, }) {
    ...
    function buildRemoteValidation(snapshot = {}) {
        return {
    ...
    async function getOpenClawConfigState(options = {}) {
        if (remoteTarget) {
    ...
    async function getRemoteOpenClawConfigStateWithRetry(options = {}, retryOptions = {}) {
    ...
    async function applyOpenClawConfigPatch({ agentId = '', baseHash = '', remoteAuthorization = null, restartGateway = false, values = {}, } = {}) {
        ...
        if (remoteTarget) {
            ...
            try {
                ...
                const nextCause = error;
                const nextError = createOpenClawConfigError(nextCause?.message || 'Remote OpenClaw config patch failed', Number.isInteger(nextCause?.statusCode) ? nextCause.statusCode : 500, nextCause?.errorCode || 'remote_config_patch_failed');
                throw nextError;
            }
            const healthCheck = await (0, openclaw_management_1.performHealthCheck)(config, { fetchImpl });
            const currentState = await getRemoteOpenClawConfigStateWithRetry({ agentId: normalizedAgentId }, { attempts: 8, delayMs: 1000 });
        ...
        if (!changedFields.length) {
            const currentState = await getOpenClawConfigState({ agentId: normalizedAgentId });
            const healthCheck = restartGateway ? await (0, openclaw_management_1.performHealthCheck)(config, { fetchImpl }) : null;
            return {
                ok: Boolean(currentState.validation?.ok),
                noChanges: true,
                rolledBack: false,
        ...
        await promises_1.default.mkdir(node_path_1.default.dirname(backupPath), { recursive: true });
        ...
        if (restartGateway) {
            ...
            healthCheck = await (0, openclaw_management_1.performHealthCheck)(config, { fetchImpl });
        }
        const currentState = await getOpenClawConfigState({ agentId: normalizedAgentId });
    ...
    async function restoreOpenClawConfigBackup({ agentId = '', backupId = '', remoteAuthorization = null, } = {}) {
        ...
        if (!remoteTarget) {
            ...
            try {
                if (backupEntry.backupPath) {
                    await promises_1.default.copyFile(backupEntry.backupPath, configPath);
                }
                else {
                    await promises_1.default.writeFile(configPath, backupEntry.raw, 'utf8');
                }
            }
            catch (error) {
                const nextCause = error;
                const nextError = createOpenClawConfigError(nextCause?.message || 'Local OpenClaw config rollback failed', Number.isInteger(nextCause?.statusCode) ? nextCause.statusCode : 500, nextCause?.errorCode || 'local_config_rollback_failed');
                throw nextError;
            }
            const validation = await validateConfig();
        ...
        try {
            ...
            const nextCause = error;
            const nextError = createOpenClawConfigError(nextCause?.message || 'Remote OpenClaw config rollback failed', Number.isInteger(nextCause?.statusCode) ? nextCause.statusCode : 500, nextCause?.errorCode || 'remote_config_rollback_failed');
            throw nextError;
        }
        const healthCheck = await (0, openclaw_management_1.performHealthCheck)(config, { fetchImpl });
        const currentState = await getRemoteOpenClawConfigStateWithRetry({ agentId: normalizedAgentId }, { attempts: 8, delayMs: 1000 });
